-- --- PortalVisuals_UI_Restoration_Final_v4.lua ---
-- Complete UI Architecture with Dynamic Function Keybinds, Premium Notifications, Full Background Fit, Solid Separators, Profile Card, and Scroll Bounds
...
local function SetBackgroundAsset(assetId)
    ...
    Utility:Tween(BackgroundImage, {ImageTransparency = 0.5}, 0.4, Enum.EasingStyle.Quint)
end

local function SetBackgroundMedia(url)
    local id = tostring(url)
    if id == "" or id == "0" then
        Utility:Tween(BackgroundImage, {ImageTransparency = 1}, 0.4, Enum.EasingStyle.Quint)
        task.delay(0.4, function() BackgroundImage.Image = "" end)
        return
    ...
    if string.find(id, "rbxassetid://") then
        BackgroundImage.Image = id
    elseif string.find(id, "http") then
        local decId = string.match(id, "%d+")
        if decId then
            BackgroundImage.Image = "rbxassetid://" .. decId
        else
            BackgroundImage.Image = id
        end
    else
...
local InnerContainer = Utility:Create("Frame", {Parent = Main, BackgroundTransparency = 1, BorderSizePixel = 0, Size = UDim2.new(1, 0, 1, 0), ZIndex = 1})
...
-- Profile Card Construction
local ProfileBtn = Utility:Create("TextButton", {
    Parent = LeftPanel,
    Name = "Profile",
    BackgroundTransparency = 1,
...
local AvatarBg = Utility:Create("Frame", {
    Parent = ProfileBtn,
...
local ProfileStatus = Utility:Create("TextLabel", {
    Parent = ProfileBtn,
    BackgroundTransparency = 1,
...
-- Fixed Content Area Bounds (Y = 0, H = 1, clips top and bottom properly)
...
local function CreatePage(Name)
    local Page = Utility:Create("ScrollingFrame", {
        ...
        BackgroundTransparency = 1,
        BorderSizePixel = 0,
        Position = UDim2.new(0, 0, 0, 0), -- Fixed Y bound to prevent top overflow
        Size = UDim2.new(1, 0, 1, 0), -- Full height to enforce bounds
    ...
    local Pad = Utility:Create("UIPadding", {Parent = Page, PaddingLeft = UDim.new(0, 22), PaddingRight = UDim.new(0, 22), PaddingTop = UDim.new(0, 20), PaddingBottom = UDim.new(0, 20)})
...
AddTab("Main", MainPage, false)
AddTab("Visuals", VisualsPage, false)
AddTab("Settings", SettingsPage, false)
...
-- Reworked Notification System (Stutter Fix)
...
local function Notify(title, body, duration)
    ...
    -- Smooth Slide-in Logic (Fixed Stutter)
...
CreateTextBox(BGSection, "Media URL", "https://...", "", function(text) SetBackgroundMedia(text) end)
...
for name, _ in pairs(Themes) do
    ...
    local Btn = Utility:Create("TextButton", {Parent = Frame, BackgroundColor3 = Theme.GlassCard, BackgroundTransparency = 0.6, BorderSizePixel = 0, Size = UDim2.new(1, 0, 1, 0), Font = Enum.Font.GothamBold, Text = name, TextColor3 = Theme.Text, TextSize = 14, AutoButtonColor = false, ZIndex = 6})
    ...
    Btn.MouseButton1Click:Connect(function() SetTheme(name) end)
end

local MenuKeybindFrame = CreateKeybind(SContent, "Toggle Menu Key", Enum.KeyCode.K, function(key)
    print("Menu key set: " .. key.Name)
end)
