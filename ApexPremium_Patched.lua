-- --- PortalVisuals_Fixed.lua ---
-- Fixes: keybind collision, URL bg loading, image full-fill, notification animations
-- Added: Cosmos + Nebula star themes, reworked notification system
...
-- FIX 1: Keybind table uses KeyCode.Value as key (integer), not KeyCode enum object.
-- Using the enum object as a table key causes identity mismatches on lookup.
-- All keybind ops now go through these two functions.
_G.PortalVisuals._keybinds = {}

local function SetKeybind(keyCode, callback)
    _G.PortalVisuals._keybinds[keyCode.Value] = callback
end

local function ClearKeybind(keyCode)
    _G.PortalVisuals._keybinds[keyCode.Value] = nil
end

local function FireKeybind(keyCode)
    local bind = _G.PortalVisuals._keybinds[keyCode.Value]
    if bind then
        task.spawn(bind, keyCode)
    end
end
...
local Themes = {
    Light = {
        ...
        StatValue = Color3.fromRGB(0, 100, 200), Stroke = Color3.fromRGB(200, 210, 230),
    ...
    -- NEW: Cosmos — deep space blue-black with silver accent and subtle star field
    Cosmos = {
    ...
    -- NEW: Nebula — deep magenta-purple galaxy with warm pink highlights
    Nebula = {
...
-- Star particles for Cosmos/Nebula themes
local ActiveStars = {}
...
local StarContainer -- forward declare, set after Main is created
...
-- Star container sits directly inside Main at lowest ZIndex
...
-- FIX 3: Background image parented directly to Main at ZIndex=0, sized via Heartbeat after layout.
-- The original parented to InnerContainer (ZIndex=1) and ran UpdateBackgroundSize on task.defer
-- which fires before AbsoluteSize is non-zero. Heartbeat fires after the first layout pass.
...
-- Wait for actual layout before reading AbsoluteSize
...
-- Heartbeat fires post-layout; one-shot connection until AbsoluteSize is valid
local layoutConn
...
-- FIX 2: URL background loading.
-- Roblox ImageLabel does NOT load external http URLs directly.
-- Valid inputs: rbxassetid://<number> or a raw numeric ID string.
-- This function normalizes both and handles the common mistake of passing a full URL.
local function NormalizeAssetId(raw)
    local s = tostring(raw):match("^%s*(.-)%s*$") -- trim whitespace
    if s == "" or s == "0" then return nil end
    -- Already a well-formed asset URI
    if s:match("^rbxassetid://%d+$") then return s end
    -- Bare numeric string
    if s:match("^%d+$") then return "rbxassetid://" .. s end
    -- URL containing a numeric ID anywhere — extract it
    local id = s:match("(%d+)")
...
-- Alias — same logic, same fix. External URLs are normalized to rbxassetid.
...
local function CreatePage(Name)
    local Page = Utility:Create("ScrollingFrame", {Parent = ContentArea, Name = Name.."Page", BackgroundTransparency = 1, BorderSizePixel = 0, Position = UDim2.new(0, 0, 0, 20), Size = UDim2.new(1, 0, 1, -40), CanvasSize = UDim2.new(0, 0, 0, 0), ScrollBarThickness = 3, ScrollBarImageColor3 = Theme.TextMuted, Visible = false, ZIndex = 4})
    ...
    Utility:Create("UIPadding", {Parent = Page, PaddingLeft = UDim.new(0, 22), PaddingRight = UDim.new(0, 22), PaddingTop = UDim.new(0, 4), PaddingBottom = UDim.new(0, 20)})
...
-- FIX 1 continued: CreateKeybind now uses SetKeybind/ClearKeybind with .Value keys.
-- The old code stored keybinds as _keybinds[KeyCode] which resolves to the enum object
-- as a table key — Lua compares these by identity, and a fresh lookup of Enum.KeyCode.K
-- doesn't necessarily match the object stored at set time.
    ...
    KeyBtn.MouseButton1Click:Connect(function()
        ...
        conn = UserInputService.InputBegan:Connect(function(input, gpe)
            ...
            if input.UserInputType == Enum.UserInputType.Keyboard then
                ClearKeybind(CurrentKey)
                CurrentKey = input.KeyCode
                KeyBtn.Text = CurrentKey.Name
                SetKeybind(CurrentKey, Callback)
            end
...
-- Global keybind dispatcher — uses .Value for consistent integer key lookup
UserInputService.InputBegan:Connect(function(input, gpe)
    if gpe then return end
    if input.UserInputType == Enum.UserInputType.Keyboard then
        FireKeybind(input.KeyCode)
    end
end)

-- FIX 4: Notification system rewrite.
-- Problems with original:
--   - AutomaticSize = Y with tween from height=0 animated nothing visible at first frame
--   - Tween animated both Position and Size simultaneously — conflicting with AutomaticSize
--   - No visual progress indicator
--   - Exit tween grew the card before shrinking (artifact)
-- New approach:
--   - Fixed height (72px), no AutomaticSize, so tweens work immediately
--   - Slide in from right with overshoot spring feel (Quint out)
--   - Accent progress bar drains over the duration
--   - Exit: simultaneous right-slide + fade + height collapse in sequence
...
local function Notify(title, body, duration)
    ...
    -- Wrapper controls position in list; card animates inside it
    ...
    -- Slide in with spring overshoot feel
    Utility:Tween(Card, {Position = UDim2.new(0, -4, 0, 0)}, 0.5, Enum.EasingStyle.Quint, Enum.EasingDirection.Out)
    task.delay(0.05, function()
        -- Micro-bounce settle
        TweenService:Create(Card, TweenInfo.new(0.25, Enum.EasingStyle.Back, Enum.EasingDirection.Out, 0, false, 0.45), {Position = UDim2.new(0, 0, 0, 0)}):Play()
    end)
    ...
    local function Dismiss()
        ...
        -- Collapse wrapper height so list reflows cleanly
...
-- FIX 1 continued: Menu keybind registered AFTER all other keybinds.
-- In the original, _keybinds[Enum.KeyCode.K] was set by CreateKeybind then immediately
-- overwritten by the ToggleUI assignment below. Both used the same key with no collision
-- detection, so the last write won and CreateKeybind's callback was silently dropped.
-- Now each feature owns a different key. Menu toggle is K, as before.
-- CreateKeybind for menu settings uses a separate key path and doesn't collide.
...
SetKeybind(MenuKey, ToggleUI)

local BGSection = CreateSection(SettingsPage, "Background Asset")
-- Both inputs go through the same normalized loader — URLs and IDs both work now
...
-- Menu key display keybind — uses a separate key (J) so it doesn't collide with ToggleUI on K
local MenuKeybindFrame = CreateKeybind(SContent, "Toggle Menu Key", Enum.KeyCode.J, function(key)
    -- When user reassigns, update the actual ToggleUI binding
    ClearKeybind(MenuKey)
    MenuKey = key
    SetKeybind(MenuKey, ToggleUI)
end)
