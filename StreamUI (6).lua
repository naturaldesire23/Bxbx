...
local THEME_NAMES = { "Vanilla", "Midnight", "Ocean", "Forest" }
...
-- Slow, endless colour breathing (white -> grey -> white) across every bar
-- of a createScanIcon() result.
local function cycleIconColor(bars, fromColor, toColor)
    for _, bar in ipairs(bars) do
        TweenService:Create(
            bar,
            TweenInfo.new(1.8, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut, -1, true),
            { BackgroundColor3 = toColor or Color3.fromRGB(150, 150, 150) }
        ):Play()
    end
end
