--[[
    Compkiller Interface - MODIFIED VERSION
    ...
    Version: 2.6 (FIXED)
    - FIXED: Keybinds with letters (A-Z, X, etc.)
    - ADDED: Background image support
    - IMPROVED: TextBox (password, multi-line, max length)
    - OPTIMIZED: Icon caching
    - FIXED: InputEnded handler for keybinds
--]]
...
-- ✅ FIX: Icon Cache for better performance
local IconCache = {}
...
-- ✅ FIX: Optimized icon function with caching
function Compkiller:_GetIcon(name , font_aws) : string
    ...
    if AssetId and AssetId ~= nil then
        if Compkiller.SecureMode then
            AssetId = Compkiller:CacheImage(AssetId);
        end;
...
-- ✅ FIX: Keybind display with letter support
function Compkiller:_AddLinkValue(Name , Default , GlobalBlock , LinkValues , rep , Signal)
    ...
    elseif Name == "Keybind" then
        ...
        -- ✅ FIX: GetItem with letter support
        local GetItem = function(item)
            if item then
                if typeof(item) == 'EnumItem' then
                    local name = item.Name
                    -- If it's a letter (A-Z), show it as is
                    ...
                    if #str == 1 and (str:match("%a") or str:match("%d")) then
                        return str
                    end
                    return Keys[str] or str;
                end;
            else
                return 'None';
            end;
...
-- ✅ FIX: Keybind handler with proper letter support
function Compkiller:_KeybindHandler(Parent: Frame , ObjectType: string , ElementAPI: Toggle & Slider , Signal , Zindex: number , ElementCFG: Slider)
    ...
    -- ✅ FIX: InputBegan with letter support
    UserInputService.InputBegan:Connect(function(Input,Typing)
        -- Normalize keybind value for comparison
        local keybindValue = APIRef.Keybind
        if typeof(keybindValue) == "EnumItem" then
            keybindValue = keybindValue.Name
        end
        
        -- Check for match (supports letters, numbers, special keys)
        local inputName = Input.KeyCode.Name
        local isMatch = (inputName == keybindValue) or (Input.KeyCode == APIRef.Keybind)
        
        -- Also check if it's a letter and matches
        if not isMatch and #inputName == 1 and inputName:match("%a") then
            isMatch = (inputName:upper() == keybindValue:upper())
        end
    ...
    -- ✅ FIX: InputEnded with letter support
        ...
        Compkiller:_Input(Keybind.Root,function()
            ...
            while not Selected do
                ...
                if Key.KeyCode ~= Enum.KeyCode.Unknown and not IsBlacklist(Key.KeyCode) and not IsBlacklist(Key.KeyCode.Name) then
                    ...
                    -- Allow letters, numbers, and special keys
                    if #keyName == 1 and (keyName:match("%a") or keyName:match("%d")) then
...
-- ✅ IMPROVED: TextBox with password, multi-line, max length support
function Compkiller:_LoadElement(Parent: Frame , EnabledLine: boolean , Signal , DisableStackKeybind)
    ...
    function Args:AddParagraph(Config: Paragraph) -- request by Neptune
        ...
        BlockText.TextColor3 = Compkiller.Colors.SwitchColor
        BlockText.TextSize = 14.000        BlockText.TextTransparency = 0.300
        BlockText.TextXAlignment = Enum.TextXAlignment.Left
    ...
    -- ✅ IMPROVED: TextBox with password, multi-line, max length, and numeric support
    function Args:AddTextBox(Config: TextBoxConfig)
        ...
        -- ✅ NEW: Apply TextBox features
        TextBox_2.Password = Config.Password
        TextBox_2.MultiLine = Config.MultiLine
        TextBox_2.MaxLength = Config.MaxLength

        if Config.MultiLine then
            TextBox_2.TextYAlignment = Enum.TextYAlignment.Top
            LinkValues.Size = UDim2.new(0, 150, 0, 40)
            TextBox.Size = UDim2.new(1, -1, 0, 60)
        end
...
-- ✅ ADDED: Background image support in Window
function Compkiller.new(Config : Window)
    ...
    Config = Compkiller.__CONFIG(Config , {
        ...
        TextSize = 15,
        BackgroundImage = nil, -- ✅ NEW: Background image support
    });
    ...
    if Config.BackgroundImage then
        ...
        bg.Name = Compkiller:_RandomString()
        bg.Parent = MainFrame
    ...
    function WindowArgs:DrawContainerTab(TabConfig : ContainerTab)
        ...
        Highlight.ZIndex = 2        UICorner.CornerRadius = UDim.new(0, 4)
        UICorner.Parent = Highlight
